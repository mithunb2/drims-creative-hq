...
// ── Store-level launch override (account + identity) — the file the app + worker share ──────────
let _storeOverrides = null;
/** slug -> {account_id,page_id,pixel_id,custom_conversion_id,ig_actor_id,default_landing}. Cached. */
export function loadStoreOverrides() {
  if (_storeOverrides) return _storeOverrides;
  try {
    const raw = JSON.parse(readFileSync(new URL('./store_launch_overrides.json', import.meta.url), 'utf8'));
    _storeOverrides = raw.stores || {};
  } catch { _storeOverrides = {}; }
  return _storeOverrides;
}
/** The store's launch override record (account + full identity), or null. */
export function storeOverride(job, so = null) {
  const slug = job && (job.store_slug || job.store_name);
  const map = so || loadStoreOverrides();
  return (slug && map[slug]) || null;
}

/** The account launches actually target. Precedence: per-job manual edit > store override > doc account.
 *  `so` is injectable for tests (defaults to the shared override file). */
export function effectiveAccount(job, so = null) {
  const o = (job && job.overrides) || {};
  if (o.account_id != null && o.account_id !== '') return o.account_id;   // explicit per-job edit wins
  const sov = storeOverride(job, so);
  if (sov && sov.account_id) return sov.account_id;                       // store-level redirect
  return (job && job.account_id) || null;                                // doc-resolved default
}

/** The full launch identity (page/pixel/cc/ig/landing) — from the store override when present. */
export function effectiveIdentity(job, so = null) {
  const sov = storeOverride(job, so);
  if (!sov) return null;
  return {
    account_id: sov.account_id, page_id: sov.page_id, pixel_id: sov.pixel_id,
    custom_conversion_id: sov.custom_conversion_id, ig_actor_id: sov.ig_actor_id,
    default_landing: sov.default_landing,
  };
}
...
export function allowedAccounts(job, reg) {
  ...
  const accts = (rec && rec.accounts) ? rec.accounts.slice() : [];
  // A store-level override account is an EXPLICIT, authorized redirect — include it so containment
  // (cross-store spend block) passes for it while still blocking any OTHER unlisted account.
  const sov = storeOverride(job);
  if (sov && sov.account_id && !accts.includes(sov.account_id)) accts.push(sov.account_id);
  return accts;
}
...
export function buildEffectivePlan(job, holds, reg, aslFields = null) {
  ...
  const sov = storeOverride(job);
  const isOverrideAccount = !!(sov && sov.account_id && sov.account_id === account_id);
  const owner = recordForAccount(account_id, reg);
  if (!account_id) blockers.push('no target account (store unmapped / account cleared)');
  // An authorized store-override account is valid even though it is not in the (insights) registry.
  else if (!owner && !isOverrideAccount) blockers.push(`effective account ${account_id} is not in the registry`);
  else {
    const allowed = allowedAccounts(job, reg);   // includes the override account
